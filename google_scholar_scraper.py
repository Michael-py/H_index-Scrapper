...
# Set the browser to be used (We will be using Mozilla DRIVER)
DRIVER = webdriver.DRIVER(executable_path='.\\Drivers\\geckodriver.exe')
...
for page in PAGES:
    for i in range(1, 11):
        i = str(i)
        try:
            DRIVER.find_element_by_css_selector('#gsc_sa_ccl > div:nth-child({}) > div > div > h3 > a'.format(i)).click()
            time.sleep(10)
            h_index = DRIVER.find_element_by_css_selector('#gsc_rsb_st > tbody > tr:nth-child(2)')
            name = DRIVER.find_element_by_css_selector('#gsc_prf_in')
